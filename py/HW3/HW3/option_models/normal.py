...
def normal_price(strike, spot, texp, vol, intr=0.0, divr=0.0, cp_sign=1):
...
class Model:
    ...
    def delta(self, strike, spot, texp=None, vol=None, cp_sign=1):
        ...
        '''
        d0=(self.spot-self.strike)/(self.vol*np.sqrt(texp))
        delta0=norm.cdf(d0)
        return delta0
    ...
    def vega(self, strike, spot, texp=None, vol=None, cp_sign=1):
        ...
        '''
        d0=(self.spot-self.strike)/(self.vol*np.sqrt(texp))
        vega0=np.sqrt(self.texp)*norm.pdf(d0)
        return vega0
    ...
    def gamma(self, strike, spot, texp=None, vol=None, cp_sign=1):
        ...
        '''
        d0=(self.spot-self.strike)/(self.vol*np.sqrt(texp))
        gamma0=norm.pdf(d0)/(self.vol*np.sqrt(T))
        return 0
