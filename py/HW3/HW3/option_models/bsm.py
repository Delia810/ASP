...
def bsm_price(strike, spot, texp, vol, intr=0.0, divr=0.0, cp_sign=1):
...
class Model:
    ...
    def delta(self, strike, spot, texp=None, vol=None, cp_sign=1):
        ...
        '''
        d1=np.log(spot/strike)/(self.vol*np.sqrt(texp)) + 0.5*self.vol*np.sqrt(texp)
        delta1=norm.cdf(d1)
        return delta1
    ...
    def vega(self, strike, spot, texp=None, vol=None, cp_sign=1):
        ...
        <-- PUT your implementation here
        '''
        FT=self.spot*np.sqrt(self.texp) 
        d1=np.log(spot/strike)/(self.vol*np.sqrt(texp)) + 0.5*self.vol*np.sqrt(texp)
    ...
    def gamma(self, strike, spot, texp=None, vol=None, cp_sign=1):
        ...
        n1=norm.pdf(d1)
        gamma1=n1/(self.spot*self.vol*np.sqrt(self.texp))
        return 0
